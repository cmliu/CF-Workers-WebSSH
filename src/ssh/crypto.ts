import { toBufferSource, writeUint32 } from './utils';
...
export class SSHAESGCMCipher {
  ...
  constructor(rawKey: Uint8Array, iv: Uint8Array) {
    if (iv.length !== 12) throw new Error(`AES-GCM requires a 12-byte IV, got ${iv.length}`);
   ...
   * using big-endian carry-over logic, per RFC 5647 section7.1.
   */
  private incIV(): void {
    for (let i = 11; i >= 4; i--) {
      this.iv[i]++;
  ...
  async encrypt(plaintext: Uint8Array, _seqNum?: number, aad?: Uint8Array, _commit: boolean = true): Promise<Uint8Array> {
    ...
    const encrypted = new Uint8Array(
      await crypto.subtle.encrypt(algorithm as unknown as SubtleCryptoEncryptAlgorithm, this.key, toBufferSource(plaintext))
    );
  ...
  async decrypt(ciphertext: Uint8Array, _seqNum?: number, aad?: Uint8Array, _commit: boolean = true): Promise<Uint8Array | null> {
    ...
    try {
      const decrypted = new Uint8Array(
        await crypto.subtle.decrypt(algorithm as unknown as SubtleCryptoEncryptAlgorithm, this.key, toBufferSource(ciphertext))
      );
      this.incIV();
      return decrypted;
    } catch {
...
export class SSHHMAC {
  private key: CryptoKey | null = null;
  private rawKey: Uint8Array;
  private hash: 'SHA-256' | 'SHA-512';
  readonly length: number;

  constructor(algorithm: string, rawKey: Uint8Array) {
    this.rawKey = rawKey;
    if (algorithm === 'hmac-sha2-256') {
      this.hash = 'SHA-256';
      this.length = 32;
  ...
  async sign(packet: Uint8Array, seqNum: number): Promise<Uint8Array> {
    ...
    return new Uint8Array(await crypto.subtle.sign('HMAC', this.key, toBufferSource(buildMacData(seqNum, packet))));
  }
